"""A small SQLite-backed store for local incident memory."""
...
class StoredIncident:
    """An incident and the diagnosis information retained in local memory."""
...
class SQLiteIncidentStore:
    """Persist OpenARIA incidents in a local SQLite file."""
    ...
    def _initialize(self) -> None:
        """Create the database and table when local memory is first used."""
    ...
    def _from_row(row: sqlite3.Row) -> StoredIncident:
        """Validate a stored database row as current OpenARIA models."""
