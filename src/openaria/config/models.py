"""Strict configuration contracts for OpenARIA projects and diagnosis rules."""

from typing import Literal

from pydantic import BaseModel, ConfigDict, Field

from openaria.domain import Severity

PROJECT_API_VERSION = "openaria.dev/v1alpha1"
...
class ReportsConfig(StrictModel):
    ...
    output_dir: str = Field(default=".openaria/reports", alias="outputDir")
...
class ProjectDocument(StrictModel):
    """Versioned OpenARIA project configuration document."""

    api_version: Literal["openaria.dev/v1alpha1"] = Field(alias="apiVersion")
...
class OpenARIAConfig(StrictModel):
    ...
    rules: list[DeterministicRule] = Field(default_factory=list)
    model: ModelConfig = Field(default_factory=ModelConfig)
    source_api_version: Literal["openaria.dev/v1alpha1"] = "openaria.dev/v1alpha1"
