"""OpenARIA command-line composition root."""
...
from openaria.adapters.reports import append_resolution, render_markdown_report
from openaria.adapters.sqlite import IncidentNotFoundError, SQLiteIncidentStore, search_incidents
...
def initialize(
    ...
    config_path = destination / "openaria.yml"
...
def rules_validate(
    ...
    for row in rows:
        typer.echo(
            f"- {row['id']}@{row['version']} | priority={row['priority']} | {row['classification']}"
        )
...
def _load_or_exit(config_path: Path) -> OpenARIAConfig:
    try:
...
_INITIAL_PROJECT = """apiVersion: openaria.dev/v1alpha1
kind: Project
metadata:
  name: my-pipeline
spec:
...
_INITIAL_RULES = """apiVersion: openaria.dev/v1alpha1
