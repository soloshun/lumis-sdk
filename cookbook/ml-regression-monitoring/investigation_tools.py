"""Bounded, cookbook-owned tools used by the ML-regression demonstration agent."""
...
MLContextName: TypeAlias = Literal["logs", "metrics", "lineage", "schema", "verification"]
MLRunbookName: TypeAlias = Literal["feature-drift-investigation", "model-performance-review"]
MLPlaybookName: TypeAlias = Literal["feature_drift_review"]
MLCodePath: TypeAlias = Literal[
    "src/inference.py", "src/train_regression.py", "data/housing_train.csv"
]


@dataclass(frozen=True)
class MLRegressionTools:
    """Read synthetic ML evidence and save reports without model or deployment authority."""
    ...
    def get_investigation_guide(self, incident_id: MLIncidentId) -> dict[str, object]:
    ...
    def get_context(self, incident_id: MLIncidentId, context_name: MLContextName) -> object:
        """Retrieve one named telemetry or metadata item listed in the investigation guide."""
    ...
    def read_runbook(self, name: MLRunbookName) -> dict[str, str]:
        """Read one valid cookbook-owned ML runbook named by the investigation guide."""
        return self._string_mapping(self._get(f"/knowledge/runbooks/{name}"))

    def read_playbook(self, name: MLPlaybookName) -> dict[str, str]:
        """Read the one allowlisted, recommendation-only ML playbook."""
        return self._string_mapping(self._get(f"/knowledge/playbooks/{name}"))

    def read_synthetic_code(self, file_path: MLCodePath) -> dict[str, str]:
        """Read one valid synthetic source or data file named by the investigation guide."""
        return self._string_mapping(self._get(f"/code/{file_path}"))

    def record_framework_diagnosis(self, incident_id: MLIncidentId) -> dict[str, str]:
    ...
    def propose_playbook(self, incident_id: MLIncidentId) -> dict[str, object]:
    ...
    def request_approval(self, incident_id: MLIncidentId) -> dict[str, str]:
    ...
    def export_analysis(
        self, incident_id: MLIncidentId, report_name: str, markdown: str
    ) -> dict[str, str]:
        """Create one named Markdown report from the complete final analysis.

        Use a simple `.md` filename such as `feature-contract-001-llm.md`, never a directory path.
