"""Bounded, cookbook-owned tools used by the data-pipeline demonstration agent."""
...
DataRunbookName: TypeAlias = Literal["schema-drift-investigation"]
DataPlaybookName: TypeAlias = Literal["schema_mismatch_in_dataframe"]
DataCodePath: TypeAlias = Literal["src/price_transform.py"]


@dataclass(frozen=True)
class DataPipelineTools:
    """Read synthetic evidence and persist local reports without any execution capability."""
    ...
    def get_investigation_guide(self, incident_id: DataIncidentId) -> dict[str, object]:
    ...
    def get_context(self, incident_id: DataIncidentId, context_name: DataContextName) -> object:
        """Retrieve one named synthetic context item listed in the investigation guide."""
    ...
    def read_runbook(self, name: DataRunbookName) -> dict[str, str]:
        """Read one valid cookbook-owned runbook named by the investigation guide."""
        return self._string_mapping(self._get(f"/knowledge/runbooks/{name}"))

    def read_playbook(self, name: DataPlaybookName) -> dict[str, str]:
        """Read the one allowlisted, recommendation-only cookbook playbook."""
        return self._string_mapping(self._get(f"/knowledge/playbooks/{name}"))

    def read_synthetic_code(self, file_path: DataCodePath) -> dict[str, str]:
        """Read the allowlisted synthetic source file named by the investigation guide."""
        return self._string_mapping(self._get(f"/code/{file_path}"))

    def record_framework_diagnosis(self, incident_id: DataIncidentId) -> dict[str, str]:
        """Save a deterministic framework diagnosis to this cookbook's local SQLite memory."""
    ...
    def propose_playbook(self, incident_id: DataIncidentId) -> dict[str, object]:
    ...
    def request_approval(self, incident_id: DataIncidentId) -> dict[str, str]:
        """Describe the human approval boundary; this tool cannot approve an action."""
        return {
    ...
    def export_analysis(
        self, incident_id: DataIncidentId, report_name: str, markdown: str
    ) -> dict[str, str]:
        """Create one named Markdown report from the complete final analysis.

        Use a simple `.md` filename such as `code-error-001-llm.md`, never a directory path.
