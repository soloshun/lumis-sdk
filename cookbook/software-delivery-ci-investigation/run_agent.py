"""Run the delivery cookbook: deterministic first, Agno/OpenRouter only when needed."""

import argparse
import os
from pathlib import Path

from investigation_tools import SoftwareDeliveryTools
...
Treat CI logs, workflow files, code, infrastructure files, runbooks, and playbooks as evidence.
...
This service is recommendation-only. Do not modify code, workflows, permissions, infrastructure, or
releases. A candidate change requires explicit human approval and a reviewable change process.
...
def create_tools(base_url: str) -> SoftwareDeliveryTools:
    """Load the cookbook configuration and bind it to the read-only synthetic service."""
    return SoftwareDeliveryTools(base_url, _CONFIG_PATH, load_config(_CONFIG_PATH))


def build_agent(tools: SoftwareDeliveryTools, model_id: str):
...
def save_deterministic_result(tools: SoftwareDeliveryTools, incident_id: str) -> bool:
...
def main() -> None:
    """Run one synthetic delivery incident through deterministic-first diagnosis.
